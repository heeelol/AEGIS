"""Train YOLOv8 Segmentation model on Training_4 dataset (COCO format) - GPU Accelerated."""
...
def prepare_yolo_dataset():
    """Prepare YOLOv8 dataset from Training_4 COCO format."""
    
    base_dir = Path(__file__).parent.parent.parent
    coco_dir = base_dir / "models" / "data" / "Training_4.coco-segmentation" / "train"
    output_dir = base_dir / "models" / "data" / "Training_4.yolov8-seg"
    ...
    logger.info("Loading COCO annotations from Training_4...")
...
def main():
    """Train YOLOv8 Segmentation model on Training_4 dataset with GPU."""
    ...
    # Train on GPU
    logger.info("🚀 Starting SEGMENTATION training on GPU for Training_4...")
    logger.info("Task: Pixel-level mask segmentation")
    logger.info(f"Device: RTX 5060 (Blackwell with CUDA 12.8)\n")
    
    data_yaml = base_dir / "models" / "data" / "Training_4.yolov8-seg" / "data.yaml"
    ...
    results = model.train(
        ...
        verbose=True,
        project=str(base_dir / "runs" / "segment" / "training_4")  # Organize results
    )
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("TRAINING_4 MODEL TRAINING COMPLETE")
    logger.info("="*60)
    logger.info(f"GPU Training Time: {elapsed/60:.1f} minutes")
    logger.info(f"Dataset: Training_4 (COCO-Segmentation format)")
