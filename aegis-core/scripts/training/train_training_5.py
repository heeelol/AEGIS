"""Train YOLOv8 Segmentation model on Training_5 dataset (COCO format) - GPU Accelerated."""
...
def main():
    """Train YOLOv8 Segmentation model on Training_5 dataset with GPU."""
    ...
    # Train on GPU
    logger.info("🚀 Starting SEGMENTATION training on GPU for Training_5...")
    ...
    results = model.train(
        ...
        verbose=True,
        project=str(base_dir / "runs" / "segment" / "training_5")  # Organize results
    )
    
    elapsed = time.time() - start_time
    
    # Save model
    model_path = base_dir / "models" / "custom" / "bin_segmentation_training_5.pt"
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("TRAINING_5 MODEL TRAINING COMPLETE")
    logger.info("="*60)
    logger.info(f"GPU Training Time: {elapsed/60:.1f} minutes")
    logger.info(f"Dataset: Training_5 (COCO-Segmentation format)")
