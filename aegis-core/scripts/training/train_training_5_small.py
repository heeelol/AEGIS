"""Train YOLOv8 SMALL Segmentation model on Training_5 dataset (COCO format) - GPU Accelerated."""
...
def main():
    """Train YOLOv8 SMALL Segmentation model on Training_5 dataset with GPU."""
    ...
    # Load SMALL model
    logger.info("Loading YOLOv8 SMALL SEGMENTATION model...")
    logger.info("Model: yolov8s-seg (5.1M parameters)")
    model = YOLO('yolov8s-seg.pt')
    logger.info("✓ Segmentation model loaded\n")
    
    # Train on GPU
    logger.info("🚀 Starting SMALL MODEL SEGMENTATION training on GPU for Training_5...")
    ...
    results = model.train(
        ...
        verbose=True,
        project=str(base_dir / "runs" / "segment" / "training_5_small")  # Organize results
    )
    
    elapsed = time.time() - start_time
    
    # Save model
    model_path = base_dir / "models" / "custom" / "bin_segmentation_training_5_small.pt"
