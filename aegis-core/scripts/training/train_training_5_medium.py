"""Train YOLOv8 MEDIUM Segmentation model on Training_5 dataset (COCO format) - GPU Accelerated."""
...
def main():
    """Train YOLOv8 MEDIUM Segmentation model on Training_5 dataset with GPU."""
    ...
    # Load MEDIUM model
    logger.info("Loading YOLOv8 MEDIUM SEGMENTATION model...")
    logger.info("Model: yolov8m-seg (11.8M parameters)")
    model = YOLO('yolov8m-seg.pt')
    logger.info("✓ Segmentation model loaded\n")
    
    # Train on GPU
    logger.info("🚀 Starting MEDIUM MODEL SEGMENTATION training on GPU for Training_5...")
    ...
    results = model.train(
        ...
        verbose=True,
        project=str(base_dir / "runs" / "segment" / "training_5_medium")  # Organize results
    )
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("TRAINING_5 MEDIUM MODEL TRAINING COMPLETE")
    logger.info("="*60)
    logger.info(f"GPU Training Time: {elapsed/60:.1f} minutes")
    logger.info(f"Dataset: Training_5 (100x Positional Occlusion)")
    logger.info(f"Model: YOLOv8 Medium Segmentation (11.8M params)")
