...
void playTune(const Note* tune, int len) {
  for (int i = 0; i < len; i++) {
    if (tune[i].freq > 0) tone(BUZZER_PIN, tune[i].freq, tune[i].dur);
...
void buzzerTask(void *param) {
  ...
  for (;;) {
    ...
    if (xTaskNotifyWait(0, 0xFFFFFFFF, &which, portMAX_DELAY) == pdTRUE) {
      if (which == TUNE_ERR) playTune(ERR_TUNE, ERR_LEN);
      else                   playTune(CORR_TUNE, CORR_LEN);
    }
  }
}
...
bool waitReadyAll(unsigned long timeout_ms) {
  ...
  while (millis() - start < timeout_ms) {
    ...
    for (int b = 0; b < NUM_BINS; b++) {
      if (digitalRead(bins[b].dout) == HIGH) { allReady = false; break; }
    }
...
void readGanged(long raw[NUM_BINS]) {
  ...
  for (int b = 0; b < NUM_BINS; b++) value[b] = 0;
  ...
  for (int b = 0; b < NUM_BINS; b++) {
    if (value[b] & 0x00800000UL) value[b] |= 0xFF000000UL;
...
void readAveraged(long raw[NUM_BINS], int n) {
  if (n < 1) n = 1;
  long sum[NUM_BINS];
  for (int b = 0; b < NUM_BINS; b++) sum[b] = 0;
  long one[NUM_BINS];
  for (int s = 0; s < n; s++) {
    readGanged(one);
    for (int b = 0; b < NUM_BINS; b++) sum[b] += one[b];
  }
  for (int b = 0; b < NUM_BINS; b++) raw[b] = sum[b] / n;
}
...
void tareAll(int n) {
  long raw[NUM_BINS];
  readAveraged(raw, n);
  for (int b = 0; b < NUM_BINS; b++) offset[b] = raw[b];
}
...
void loop() {
  ...
  if (cmd == "ring") {
    ...
    if (buzzerTaskHandle) xTaskNotify(buzzerTaskHandle, TUNE_CORR, eSetValueWithOverwrite);
    cmd = "null";      // one-shot: clear so it doesn't retrigger every loop
  }
  if (cmd == "err") {
    if (buzzerTaskHandle) xTaskNotify(buzzerTaskHandle, TUNE_ERR, eSetValueWithOverwrite);
    cmd = "null";
  }
