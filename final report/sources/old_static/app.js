...
function makeBinTile(binId, b) {
  ...
  if (status === "missing") {
    ...
    inner += '<div class="bin-id">' + label + '</div>';
    if (returnAmt(b) > 0) {
      inner += '<div style="background: #8B2020; color: #9A5B5B; text-shadow: -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000; font-size: 8px; padding: 2px 4px; border-radius: 4px; margin-top: 4px; white-space: nowrap;">RETURN ' + returnAmt(b) + '</div>';
    }
    inner += '<div class="quantity" style="margin-top: 4px;">' + Math.min(b.placed, b.target) + '/' + b.target + '</div>';
  }
