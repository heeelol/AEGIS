"""Train YOLOv8 Segmentation model on Training_2 dataset (COCO format) - GPU Accelerated."""
...
def prepare_yolo_dataset():
    """Prepare YOLOv8 dataset from Training_2 COCO format."""
    
    base_dir = Path(__file__).parent.parent.parent
    coco_dir = base_dir / "models" / "data" / "Training_2.coco-segmentation" / "train"
    output_dir = base_dir / "models" / "data" / "Training_2.yolov8-seg"
    ...
    logger.info("Loading COCO annotations from Training_2...")
...
def main():
    """Train YOLOv8 Segmentation model on Training_2 dataset with GPU."""
    ...
    # Train on GPU
    logger.info("🚀 Starting SEGMENTATION training on GPU for Training_2...")
    logger.info("Task: Pixel-level mask segmentation")
    logger.info(f"Device: RTX 5060 (Blackwell with CUDA 12.8)\n")
    
    data_yaml = base_dir / "models" / "data" / "Training_2.yolov8-seg" / "data.yaml"
    ...
    results = model.train(
        ...
        verbose=True,
        project=str(base_dir / "runs" / "segment" / "training_2")  # Organize results
    )
    
    elapsed = time.time() - start_time
    
    # Save model
    model_path = base_dir / "models" / "custom" / "bin_segmentation_training_2.pt"
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("TRAINING_2 MODEL TRAINING COMPLETE")
    logger.info("="*60)
    logger.info(f"GPU Training Time: {elapsed/60:.1f} minutes")
    logger.info(f"Dataset: Training_2 (COCO-Segmentation format)")
