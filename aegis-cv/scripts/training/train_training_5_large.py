"""Train YOLOv8 LARGE Segmentation model on Training_5 dataset (COCO format) - GPU Accelerated."""
...
def prepare_yolo_dataset():
    """Prepare YOLOv8 dataset from Training_5 COCO format."""
    
    base_dir = Path(__file__).parent.parent.parent
    coco_dir = base_dir / "models" / "data" / "Training_5.coco-segmentation" / "train"
    output_dir = base_dir / "models" / "data" / "Training_5.yolov8-seg"
    ...
    logger.info("Loading COCO annotations from Training_5...")
...
def main():
    """Train YOLOv8 LARGE Segmentation model on Training_5 dataset with GPU."""
    ...
    # Load LARGE model
    logger.info("Loading YOLOv8 LARGE SEGMENTATION model...")
    logger.info("Model: yolov8l-seg (52.3M parameters)")
    model = YOLO('yolov8l-seg.pt')
    logger.info("✓ Segmentation model loaded\n")
    
    # Train on GPU
    logger.info("🚀 Starting LARGE MODEL SEGMENTATION training on GPU for Training_5...")
    logger.info("Task: Pixel-level mask segmentation")
    logger.info(f"Device: RTX 5060 (Blackwell with CUDA 12.8)\n")
    
    data_yaml = base_dir / "models" / "data" / "Training_5.yolov8-seg" / "data.yaml"
    ...
    results = model.train(
        ...
        epochs=50,
        imgsz=640,
        batch=8,  # Reduced batch size for large model (VRAM constraint)
        device=0,  # Use GPU device 0
        amp=True,  # Automatic mixed precision for faster training
        patience=10,
        task='segment',  # Segmentation task
        verbose=True,
        project=str(base_dir / "runs" / "segment" / "training_5_large")  # Organize results
    )
    
    elapsed = time.time() - start_time
    
    # Save model
    model_path = base_dir / "models" / "custom" / "bin_segmentation_training_5_large.pt"
